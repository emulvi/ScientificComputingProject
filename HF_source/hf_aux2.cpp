...
void read_v_int(int ao, Real_4dMatrix& v_int){
   ...
   cout << "h" << endl;
   vector<int> jvec(size,0);
   cout << "e" << endl;
   vector<int> kvec(size,0);
   cout << "l" << endl;
   vector<int> lvec(size,0);
   cout << "l" << endl;
   vector<double> val(size,0.0);
   cout << "o" << endl;
   
   for (int i=0; i<size; i++){
     interaction >> ivec[i] >> jvec[i] >> kvec[i] >> lvec[i] >> val[i];
     }*/
   ...
   /*cout << "w" << endl;
   for (int i=0;i<size; i++){
     v_int[ivec[i]-1][jvec[i]-1][kvec[i]-1][lvec[i]-1] = val[i];
     }
     cout << "o" << endl;*/
...
void build_P(int ao, int occ, Matrix& C_ao, Matrix &P0){
  ...
  cout << "Initial Density matrix is: " << "\n" << P0 << endl; 
   return;
};
...
void build_new_Fock(int ao, Matrix& P0, Real_4dMatrix& v_int, Matrix&H_core, Matrix& Fock_new){
      ...
      for (int j=0; j<ao ; j++){
         ...
         for (int k=0; k<ao ; k++){
             for (int l=0; l<ao ; l++){
                 ...
                 Fock_new(i,j)=Fock_new(i,j) + P0(k,l)*(2.*v_int[i][j][k][l]-v_int[i][l][k][j]);
